...
function uniqueSubjects(
  ...
  const filtered =
    semesterNumber !== undefined
      ? curriculum?.filter((c) => c.semesterNumber === semesterNumber)
      : curriculum;
  const subjects =
    filtered
      ?.map((c) => c.subject)
...
function uniqueSemesters(
  curriculum: { semesterNumber: number }[] | undefined,
): number[] {
  if (!curriculum?.length) return [];
  return [...new Set(curriculum.map((c) => c.semesterNumber))].sort((a, b) => a - b);
}
...
export default function ProfilePage() {
  ...
  } = useCatalogCurriculum(effectiveMajorId || undefined);

  const availableSemesters = useMemo(
    () => uniqueSemesters(curriculum),
    [curriculum],
  );
  ...
  const availableSubjects = useMemo(
    () =>
      uniqueSubjects(
        curriculum,
        effectiveSemesterNumber ? Number(effectiveSemesterNumber) : undefined,
      ),
    [curriculum, effectiveSemesterNumber],
  );

  const selectionMatchesProfile =
    effectiveMajorId === majorFromProfile &&
    effectiveSemesterNumber === semesterFromProfile;
            ...
            <div>
              ...
              <div className="mt-2 rounded-xl border-2 border-brutal-ink bg-brutal-bg p-3">
                {isProfileLoading || (effectiveMajorId && isCurriculumLoading) ? (
                  ...
                  <p className="text-sm text-brutal-muted">
                    Ngành này chưa có môn trong chương trình đào tạo.
                  </p>
